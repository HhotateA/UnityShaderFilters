    ...
    SubShader {
        ...
        Pass {
            ...
            fixed4 frag(v2f IN) : SV_Target {
                ...
                float4 samplecol = col[0]+col[1]+col[2]+col[3]+col[5]+col[6]+col[7]+col[8]
                                  -col[4]*8;
                float3 weight;
                #ifdef Step
                    weight = step(0.1,saturate(samplecol.rgb));
                #else
                    weight = saturate(samplecol.rgb);
                #endif
                float3 output = float3( lerp(_backcolor.r,getCol(screenPos).r,weight.x),
                                        lerp(_backcolor.g,getCol(screenPos).g,weight.y),
                                        lerp(_backcolor.b,getCol(screenPos).b,weight.z));
